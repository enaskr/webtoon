...
<?php
	...
	$isAlreadyView = $isAlreadyView." WHERE UV.MBR_NO = (SELECT UI.MBR_NO FROM USER_INFO UI WHERE UI.MBR_NO=UV.MBR_NO AND UI.USER_ID='".$MBR_USER_ID."' AND UI.USE_YN='Y') AND UV.SITE_ID = '".$siteId."' AND UV.USE_YN='Y' ";
	...
	if ( strlen($recentToons) > 1 ) {
		send_discord($webhookURL, $SITE_ID." ".strtoupper($type)." 업데이트 완료!", $MBR_USER_ID, $SITE_ID." ".strtoupper($type)." 업데이트 내역", $recentToons, $webpath."/".$type."/".strtolower($SITE_ID)."/", "3366ff");
	}
?>
